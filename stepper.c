...
void stepper_manualstep(uint8_t idxMotorv, uint8_t motorDirectionv) {
  idxMotor = idxMotorv;
  motorDirection = motorDirectionv;
}
...
void stepper_interrupt()
{
  ...
  if (stepper_mode == STEPPERMODE_AUTO) {
  ...
  */
    ...
    if (idxMotor != IDX_MOTORNONE) {
      motor_step(idxMotor, motorDirection);
      idxMotor = IDX_MOTORNONE;
    }
  }
